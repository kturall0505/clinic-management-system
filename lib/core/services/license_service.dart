...
/// Local-first licensing: the app works offline, but must reach the vendor's
/// license server at least once every [gracePeriod]. Only the tenant id and a
/// heartbeat timestamp are sent — never any clinical data.
class LicenseService extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    if (_lastSuccess == null) {
      // First installation: grant one grace period starting from install time
      // so the clinic can finish setup before the first online check.
